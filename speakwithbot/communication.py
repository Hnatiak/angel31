    ...
    if any(command in text for command in hello):
        ...
        for keyword in angel:
            if text == f"{keyword} представся" or text == f"{keyword} представлення" or text == f"{keyword} хто ти" or text == f"{keyword} команди" or text == f"{keyword} що вмієш":
                ...
                bot.send_message(message.chat.id,
                                 'Вибач, але я не можу надавати інформацію про покупку піару або адмінки. Я розроблений, щоб надавати інформацію та відповідати на запитання. Якщо у вас є будь-які питання, я готовий вам допомогти!')
            else:
                bot.send_message(message.chat.id, 'Вибач, я не зрозумів ваше повідомлення. Можливо, ви можете переформулювати своє запитання або скористатися іншою командою?')
